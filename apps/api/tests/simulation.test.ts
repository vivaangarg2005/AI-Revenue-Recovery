...
describe("RECOVER-AI 500-Case Simulation Experiment Test Suite", () => {
  it("1 & 2. Should generate 500 cases split 250 Control / 250 Treatment", () => {
    const { controlCases, treatmentCases } =
      SimulationGenerator.generateBatch(20260822);
  ...
  it("3 & 4. Deterministic seed must produce 100% identical dataset and metrics", async () => {
    ...
    expect(run1.metrics.controlNetRecoveredPaise).toBe(
      run2.metrics.controlNetRecoveredPaise,
    );
    expect(run1.metrics.treatmentNetRecoveredPaise).toBe(
      run2.metrics.treatmentNetRecoveredPaise,
    );
    expect(run1.metrics.aiDiagnosisAccuracyPercent).toBe(
      run2.metrics.aiDiagnosisAccuracyPercent,
    );
  });
  ...
  it("5. Different seed must produce different dataset and metrics", async () => {
    ...
    expect(runA.metrics.controlNetRecoveredPaise).not.toBe(
      runB.metrics.controlNetRecoveredPaise,
    );
  });
  ...
  it("6. Ground truth must be preserved and independent of AI", () => {
    const { controlCases, treatmentCases } =
      SimulationGenerator.generateBatch(20260822);
    expect(controlCases[0].groundTruth.actualFailureCategory).toBe(
      treatmentCases[0].groundTruth.actualFailureCategory,
    );
    expect(controlCases[0].groundTruth.canRecover).toBe(
      treatmentCases[0].groundTruth.canRecover,
    );
  });

  it("7 & 8. Control and Treatment cohorts must execute successfully", async () => {
    const { controlOutcomes, treatmentOutcomes } =
      await SimulationRunner.runSimulation(20260822);
  ...
  it("9 & 10 & 13. Money calculations must use integer BigInt Paise and net = gross - discount", async () => {
    const { metrics, treatmentOutcomes } =
      await SimulationRunner.runSimulation(20260822);
  ...
  it("12. AI Diagnosis Accuracy must be calculated against ground truth", async () => {
    ...
    const expectedAccuracy = Number(
      ((correct / treatmentOutcomes.length) * 100).toFixed(1),
    );
